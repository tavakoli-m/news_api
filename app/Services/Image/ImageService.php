...
class ImageService extends ImageToolsService
{
    ...
    public function __construct(private $manager = new ImageManager(new Driver())) {}
    ...
    public function save($image)
    {
        ...
        $this->provider();
        //save image
        $result = $this->manager->read($image->getRealPath())->save(public_path($this->getImageAddress()), null, $this->getImageFormat());
    ...
    public function fitAndSave($image, $width, $height)
    {
        ...
        $this->provider();
        //save image
        $result = $this->manager->read($image->getRealPath())->resize($width, $height)->save(public_path($this->getImageAddress()), null, $this->getImageFormat());
        return $result ? $this->getImageAddress() : false;
    }
    ...
    public function createIndexAndSave($image)
    {
        ...
        foreach ($imageSizes as $sizeAlias => $imageSize) {
            ...
            //save image
            $result = $this->manager->read($image->getRealPath())->resize($imageSize['width'], $imageSize['height'])->save(public_path($this->getImageAddress()), null, $this->getImageFormat());
            if ($result)
